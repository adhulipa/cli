...
func runPrune(dockerCli command.Cli, options pruneOptions) error {
	...
	imagePrune := func(dockerCli command.Cli, dryRun bool, filter opts.FilterOpt) (uint64, string, error) {
		return image.RunPrune(dockerCli, options.all, options.dryRun, options.filter)
	}
